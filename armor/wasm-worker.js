...
function wrapExports({ compute_results }) {
    return ({ width, height, maxIterations }) => {
        const start = performance.now();
        const rawImageData = compute_results(width, height, maxIterations);
...
async function initHandlers() {
    let [singleThread, multiThread] = await Promise.all([
        (async () => {
            const singleThread = await import('./pkg/armor.js');
