...
pub async fn compute_results(config: JsValue, n: usize) -> Result<(), ComputeError> {
    ...
    let selected_exotic = if let ExoticChoiceModel::Some(exotic) = config.selected_exotic {
        let tx = db.transaction(&["manifestArmor"], TransactionMode::ReadOnly).map_err(|e| ComputeError::LoadDataError)?;
        let manifest_armor = tx.store("manifestArmor").unwrap();
        let armor = manifest_armor.index("hash").unwrap().get(&JsValue::from(exotic)).await.map_err(|e| ComputeError::LoadDataError)?;
        let armor: ManifestArmor = serde_wasm_bindgen::from_value(armor).map_err(|e| ComputeError::LoadDataError)?;
    ...
    let armor: Vec<InventoryArmor> = {
        let tx = db.transaction(&["inventoryArmor"], TransactionMode::ReadOnly).map_err(|e| ComputeError::LoadDataError)?;
        let inventory_armor = tx.store("inventory_armor").map_err(|e| ComputeError::LoadDataError)?;
        let key_range = KeyRange::only(&(config.character_class as u32).into()).unwrap();
        let armor = inventory_armor.index("clazz").unwrap();
        let armor = armor.get_all(Some(&key_range), None, None, None).await.map_err(|e| ComputeError::LoadDataError)?;
