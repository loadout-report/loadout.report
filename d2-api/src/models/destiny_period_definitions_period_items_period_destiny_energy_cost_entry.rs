/*
 * Bungie.Net API
 *
 * These endpoints constitute the functionality exposed by Bungie.net, both for more traditional website functionality and for connectivity to Bungie video games and their related functionality.
 *
 * The version of the OpenAPI document: 2.16.0
 * Contact: support@bungie.com
 * Generated by: https://openapi-generator.tech
 */

/// DestinyPeriodDefinitionsPeriodItemsPeriodDestinyEnergyCostEntry : Some plugs cost Energy, which is a stat on the item that can be increased by other plugs (that, at least in Armor 2.0, have a \"masterworks-like\" mechanic for upgrading). If a plug has costs, the details of that cost are defined here.



#[derive(Clone, Debug, PartialEq, Default, Serialize, Deserialize)]
pub struct DestinyPeriodDefinitionsPeriodItemsPeriodDestinyEnergyCostEntry {
    /// The Energy cost for inserting this plug.
    #[serde(rename = "energyCost", skip_serializing_if = "Option::is_none")]
    pub energy_cost: Option<i32>,
    /// The type of energy that this plug costs, as a reference to the DestinyEnergyTypeDefinition of the energy type.
    #[serde(rename = "energyTypeHash", skip_serializing_if = "Option::is_none")]
    pub energy_type_hash: Option<i32>,
    /// The type of energy that this plug costs, in enum form.
    #[serde(rename = "energyType", skip_serializing_if = "Option::is_none")]
    pub energy_type: Option<i32>,
}

impl DestinyPeriodDefinitionsPeriodItemsPeriodDestinyEnergyCostEntry {
    /// Some plugs cost Energy, which is a stat on the item that can be increased by other plugs (that, at least in Armor 2.0, have a \"masterworks-like\" mechanic for upgrading). If a plug has costs, the details of that cost are defined here.
    pub fn new() -> DestinyPeriodDefinitionsPeriodItemsPeriodDestinyEnergyCostEntry {
        DestinyPeriodDefinitionsPeriodItemsPeriodDestinyEnergyCostEntry {
            energy_cost: None,
            energy_type_hash: None,
            energy_type: None,
        }
    }
}


